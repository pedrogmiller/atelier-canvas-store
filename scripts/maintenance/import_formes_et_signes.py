...
# Diptych (01 + 02) - 1 Panoramic Print containing 2 drawings
...
w, h = art1.size
gap = int(w * 0.08)
diptych_sheet = Image.new("RGB", (w * 2 + gap, h), (248, 246, 240))
diptych_sheet.paste(art1, (0, 0))
diptych_sheet.paste(art2, (w + gap, 0))

margin_x = int(diptych_sheet.width * 0.05)
margin_y = int(diptych_sheet.height * 0.06)
diptych_master = ImageOps.expand(diptych_sheet, border=(margin_x, margin_y), fill=(248, 246, 240))
...
# Generate Diptych mockups as 1 single framed panoramic print
for f_key, cfg in MockupAgent.FRAME_CONFIGS.items():
    ...
    cd_img = mockup_agent._create_corner_detail_mockup(diptych_master, frame_type=f_key, cfg=cfg)
    cd_img.save(diptych_dir / f"corner_{f_key}.jpg", "JPEG", quality=92)
    
    lr_img = mockup_agent._create_credenza_living_room_mockup(diptych_master, frame_type=f_key, cfg=cfg)
...
diptych_entry = {
    ...
    "short_summary": "The Signature Diptych (Set of 2). The perfect dialogue of blues: pair L'Envol Indigo and Bleu Lunaire side-by-side on one curated fine art poster with an automatic 15% bundle savings.",
    "story_and_concept": "Curated pairing of L'Envol Indigo and Bleu Lunaire on a single museum-grade archival fine art print sheet. A harmonious study in smoked indigo, ultramarine cobalt, and Tuscan ochre on archival cotton rag paper.",
    "styling_tips": "Hang horizontally above a master bed, living room sectional, or dining sideboard.",
    "specifications": {
        "paper": collection_info["paper_spec"],
        "framing": "Includes solid FSC-certified wood frame or unframed archival print",
        "printing": "12-color archival Giclée pigment printing",
        "fulfillment": "Locally printed and packaged across 32 regional global hubs"
    },
...
# Composite Triptych master art - 1 Panoramic Print containing 3 drawings
triptych_sheet = Image.new("RGB", (w * 3 + gap * 2, h), (248, 246, 240))
triptych_sheet.paste(art1, (0, 0))
triptych_sheet.paste(art2, (w + gap, 0))
triptych_sheet.paste(art3, ((w + gap) * 2, 0))

margin_tx = int(triptych_sheet.width * 0.04)
margin_ty = int(triptych_sheet.height * 0.06)
triptych_master = ImageOps.expand(triptych_sheet, border=(margin_tx, margin_ty), fill=(248, 246, 240))
...
# Generate Triptych mockups as 1 single framed panoramic print
for f_key, cfg in MockupAgent.FRAME_CONFIGS.items():
    ...
    cd_img = mockup_agent._create_corner_detail_mockup(triptych_master, frame_type=f_key, cfg=cfg)
    cd_img.save(triptych_dir / f"corner_{f_key}.jpg", "JPEG", quality=92)
    
    lr_img = mockup_agent._create_credenza_living_room_mockup(triptych_master, frame_type=f_key, cfg=cfg)
...
triptych_entry = {
    ...
    "short_summary": "The Collector's Triptych (Set of 3). The complete 3-piece gallery composition spanning midnight indigo, vibrant cobalt, and warm Tuscan ochre in one master fine art print with 20% bundle savings.",
    "story_and_concept": "Curated 3-drawing lithographic triptych combining L'Envol Indigo, Bleu Lunaire, and Ocre Cosmique in a single panoramic fine art print.",
    "styling_tips": "Mount horizontally over large sofas, living room credenzas, or office spaces.",
    "specifications": {
        "paper": collection_info["paper_spec"],
        "framing": "Includes solid FSC-certified wood frame or unframed archival print",
        "printing": "12-color archival Giclée pigment printing",
